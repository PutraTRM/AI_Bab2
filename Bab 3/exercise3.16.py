...
# Modified for 8-state routing problem
...
# Create an 8-state routing list and set the goal
points_list = [
    (0, 1),
    (0, 2),
    (1, 3),
...
for i in range(700):
    ...
    # Select a random current state
    current_state = np.random.randint(
        0, int(Q.shape[0])
    )

    # Work out all available next step actions
    available_act = available_actions(
        R, current_state
    )
